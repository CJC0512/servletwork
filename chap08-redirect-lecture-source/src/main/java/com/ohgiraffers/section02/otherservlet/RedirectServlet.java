...
public class RedirectServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        /* 설명. 파라미터가 넘어오면 request 객체가 달라ㅏ져도 값이 전달되어 온다. */
        System.out.println("리다이렉트 이후 request의 attribute: " + req.getParameter("test"));

        /* 설명.
        *   redirect 시에는 request의 attribute 개념으로 다른 서블릿에게 값(상태)을 전달할 수 없다.
        *   왜냐하면 다음 서블릿으로 클라이언트가 새로 요청하면 기존의 request와 response 객체가 아닌
        *   새로운 request와 response 객체가 만들어지며 요청하기 떄문이다.
        *
        *  설명.
