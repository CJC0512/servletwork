...
public class LifeCycleTestServlet extends HttpServlet {
    ...
    /* 설명.
    *   서블릿 컨테이너에 의해 호출되며 최초 쵸옃 시에만 inti() 이후에 동작하고,
    *   두 번째 요청부터는 바로 service()만 호출하게 된다
    * */
    /* 필기. request 사용자가 넣은 입력값, response 사용자가 어디에서 요청했는지(Who) */
    @Override
    public void service(ServletRequest request, ServletResponse response)
            throws ServletException, IOException {
        System.out.println("xml 매핑 service() 메소드 호출!!");
        System.out.println("실제로는 이 곳에서 요청 방식에 따라 doGet() 도는 doPost()가 실행됨");
    }
